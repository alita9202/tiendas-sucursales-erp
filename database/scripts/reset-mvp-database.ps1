...
Write-Host "Dropping MVP database if exists..." -ForegroundColor Yellow
Invoke-PsqlSql -Database "postgres" -Sql "DROP DATABASE IF EXISTS supermarket_mvp_db WITH (FORCE);"
...
Invoke-PsqlFile -Database "supermarket_mvp_db" -FilePath (Join-Path $MvpPath "01-schema.sql")

Write-Host "Installing functions..." -ForegroundColor Cyan
Invoke-PsqlFile -Database "supermarket_mvp_db" -FilePath (Join-Path $MvpPath "04-functions.sql")

Write-Host "Installing views..." -ForegroundColor Cyan
Invoke-PsqlFile -Database "supermarket_mvp_db" -FilePath (Join-Path $MvpPath "03-views.sql")

Write-Host "Loading demo seed..." -ForegroundColor Cyan
Invoke-PsqlFile -Database "supermarket_mvp_db" -FilePath (Join-Path $MvpPath "02-seed-demo.sql")

Write-Host ""
Write-Host "MVP database reset completed successfully." -ForegroundColor Green
